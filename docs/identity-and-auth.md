...
- **Actor / Principal** (`axp.Actor`, re-exported as `auth.Principal`) — the identity itself: `id` (`kind:name`, e.g. `agent::mac`), `kind` (human/host/machine/agent/service/automation/api/mcp/plugin), optional `display_name`. Stable IDs, never a display name alone.
