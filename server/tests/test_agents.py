...
async def test_create_and_get_agent(client, auth_header):
    headers = auth_header()
    resp = await client.post("/api/v1/agents", json={"name": "Buddy", "provider": "claude"}, headers=headers)
...
async def test_create_agent_identity(client, auth_header):
    headers = auth_header()
    agent = (await client.post("/api/v1/agents", json={"name": "Buddy", "provider": "claude"}, headers=headers)).json()
    ...
    assert identity["identity_key"].startswith("agent:claude:")
...
async def test_assign_existing_identity_key(client, auth_header):
    ...
    resp = await client.post(
        ...
        json={"identity_key": "agent:claude:mac"},
        headers=headers,
    )
    assert resp.status_code == 201
    assert resp.json()["identity_key"] == "agent:claude:mac"
