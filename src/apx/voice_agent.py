...
def set_fallback(agent: str) -> str:
    ...
    if agent == "apx":
        ...
        return "Choose APX, Claude, or Codex."
...
def reasoning_fallback(text: str) -> str:
    ...
    if agent == "apx":
        return (
            ...
            "You can choose Claude or Codex as the reasoning fallback."
        )
...
def route(text: str) -> str:
    ...
    if "use claude" in lower:
        return set_fallback("claude")
    ...
    if any(
        ...
        open_terminal_command("exec claude")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if command == "agent":
        if not args:
            raise SystemExit(
                "Usage: apx voice agent apx|claude|codex"
            )
    ...
    print(
        ...
        "  apx voice agent apx|claude|codex\n"
        "  apx voice doctor\n"
        "  apx voice self-test\n"
    )
    return 0
