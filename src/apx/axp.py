...
# matching confirmation in the ActionRequest or APX returns action.awaiting_approval.
...
PUBLIC_ACTION_STATUSES = (
    "available", "unavailable",
    "pending", "completed", "failed", "cancelled", "rejected",
    "awaiting-approval", "awaiting-input", "scheduled", "denied"
)

INTERNAL_EXECUTION_STATUSES = (
    "prepared", "accepted", "in-progress", "authorized", "verification_failed"
)
...
EVENT_NAMES = ("policy.allowed","policy.denied","system.state_changed","security.incident_started","security.lockdown_started","security.lockdown_ended","security.break_glass_started","secret.updated","secret.rotated","secret.revoked","credential.rotation_started","credential.rotation_completed","credential.rotation_failed","actor.connected",
    ...
    "action.prepared","action.awaiting_approval","action.authorized","action.started","action.completed","action.failed","action.reversed",
...
class StructuredError:
    ...
    def from_dict(cls, value: dict[str, Any]) -> "StructuredError":
        known = {"code", "message", "details", "retryable", "provider_code", "retry_after", "next_actions"}
...
class ActionDefinition:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {"apx":APX_PROTOCOL_VERSION,"type":"action.definition",**result}
...
class ActionRequest:
    ...
    protocol_version: str = APX_PROTOCOL_VERSION
...
class ActionReceipt:
    ...
    def from_dict(cls, value: dict[str, Any]) -> "ActionReceipt":
        known = {
            "action", "provider", "target", "actor", "status", "result", "receipt_id", "request_id",
            "effective_time", "verification_status", "reversible", "reverse_action", "provider_reference",
            "side_effects", "reversal", "timestamp", "delegated_by", "prepared_action_id", "committed_at",
            "completed_at", "verified_state", "postconditions", "partial_effects", "digest", "signature",
            "signer_node", "key_id"
        }
        raw = {k: v for k, v in value.items() if k in known}
...
class ActionResult:
    ...
    def from_dict(cls, value: dict[str, Any]) -> "ActionResult":
        ...
        receipt=ActionReceipt.from_dict(value["receipt"]) if value.get("receipt") else None
...
class PreparedAction:
    ...
    def from_dict(cls, value: dict[str, Any]) -> "PreparedAction":
        known = {
            "action", "target", "input", "effect", "confirmation_required", "cost", "reversible",
            "reverse_action", "expires_at", "request_id", "provider", "side_effects", "provider_conditions",
            "recurring_terms", "authorization", "confirmation_terms", "prepared_action_id", "created_at",
