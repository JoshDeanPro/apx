...
it is not how APX wires into an agent that supports something more direct. Claude
...
Claude Code is the first fully-supported target (a `/apx` slash command +
a Skill, both under `.claude/`, both stable documented Claude Code conventions).
...
# Codex Skills use the same frontmatter shape as Claude Code's (verified against
...
def install_claude_code(*, root: Path, global_scope: bool = False) -> dict[str, Any]:
    """Writes `.claude/commands/apx.md` (the `/apx` slash command) and
    `.claude/skills/apx/SKILL.md`. Both are plain files under an agent-owned
    directory Claude Code already scans -- no daemon, no registration step,
    no protocol handshake."""
    claude_root = (Path.home()/".claude") if global_scope else (root/".claude")
    command_path = claude_root/"commands"/"apx.md"
    skill_path = claude_root/"skills"/"apx"/"SKILL.md"
    command_path.parent.mkdir(parents=True, exist_ok=True)
    skill_path.parent.mkdir(parents=True, exist_ok=True)
    command_path.write_text(SLASH_COMMAND_CONTENT, encoding="utf-8")
    skill_path.write_text(SKILL_CONTENT, encoding="utf-8")
    return {"agent": "claude-code", "scope": "user" if global_scope else "project",
...
def install_codex(*, root: Path, global_scope: bool = False) -> dict[str, Any]:
    ...
    Claude Code has `.claude/skills` inside a repo) -- `global_scope` is
    ...
    No `/apx` slash command here: unlike Claude Code, Codex has no verified
    ...
    functional apx access Claude Code's Skill gives it; it is just reached
...
# this machine when checked, unlike Claude Code and Codex, both confirmed
# real and present).
