...
What that service actually does, and what this module reproduces: a ``
...
mid-iteration, real usage-limit-reset parsing ( Code reports a rate-limited
...
# "" is the exact recipe from the real palis-autopilot.service invocation
...
AGENT_RUNTIMES: dict[str, dict[str, Any]] = {
    "": {
        "verified": True,
        "bin_resolve": r'''_BIN="$(command -v  2>/dev/null || true)"
if [[ -z "$_BIN" || ! -x "$_BIN" ]]; then
  log "FATAL:  executable not found on PATH"; exit 127
fi''',
...
if [[ -z "$CODEX_BIN" || ! -x "$CODEX_BIN" ]]; then
  ...
  # installed version before relying on this in production. Unlike the 
  ...
  run_with_timeout "$ITERATION_TIMEOUT" \
    "$CODEX_BIN" exec \
      ...
      "$(cat "$PROMPT_FILE")" \
    > "$iter_log" 2>&1''',
        "rate_limit_funcs": r'''# Codex's rate-limit-event wire shape is not verified the way  Code's
...
def render(name: str, *, repo: str, description: str = "", project_description: str = "", user: str = "root",
           init_system: str = "systemd", runtime: str = "", model: str = "opus", effort: str = "medium",
