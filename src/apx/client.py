...
from .providers import DISCOVERY_PATH, ProviderManifest, evaluate_compatibility, CompatibilityResult
...
class HTTPClientTransport:
    ...
    def receipt(self, receipt_id):
        ...
        return None if value.get("error") else ActionReceipt.from_dict(value)
...
class APXClient:
    ...
    def __init__(self, transport: ClientTransport, client_context: dict[str, Any] | None = None):
    ...
    def discover(self):
        self._manifest = self.transport.discover()
        self._compatibility = evaluate_compatibility(self.client_context, self._manifest)
        return self._manifest

    def check_compatibility(self) -> CompatibilityResult:
        if not self._compatibility: self.discover()
        return self._compatibility

    def actions(self):
        if not self._manifest: self.discover()
    ...
    def prepare(self, action: str, *, target=None, input=None, **envelope):
        ...
        if not compat.compatible:
            print("COMPATIBILITY FAILED:", compat.reasons)
            return ActionResult(action, False, error=StructuredError("incompatible", "; ".join(compat.reasons)))
