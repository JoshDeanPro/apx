...
integration that already exists elsewhere. `~/.claude.json` (Claude Code) and
...
class ClaudeCodeSource(IngestSource):
    id = "claude_code"; label = "Claude Code (~/.claude.json)"; verified = True

    def __init__(self, path: Path | None = None): self.path = path or Path.home()/".claude.json"
...
# on a real machine the way claude_code's and codex's were. "kimi_code" and
