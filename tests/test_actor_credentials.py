...
class ActorCredentialStoreTests(unittest.TestCase):
    ...
    def test_issue_defaults_to_active_with_no_plaintext(self):
        record=self.store.issue("agent:claude:mac",fingerprint=fingerprint("super-secret-value"))
        self.assertEqual(record["state"],"active")
        self.assertNotIn("super-secret-value",json.dumps(record))

    def test_invalid_type_rejected(self):
        with self.assertRaises(ActorCredentialError): self.store.issue("agent:claude:mac",type="not-a-real-type")

    def test_rotate_keeps_old_active_material_marked_rotating_not_revoked(self):
        original=self.store.issue("agent:claude:mac",fingerprint=fingerprint("v1"))
    ...
    def test_cannot_rotate_a_non_active_credential(self):
        original=self.store.issue("agent:claude:mac")
...
class ActorCredentialActionsTests(unittest.TestCase):
    ...
    def test_issue_rotate_confirm_revoke_via_actions(self):
        issued=self.cloud.run("credential.issue",principal="agent:claude:mac",fingerprint="fp1").result
    ...
    def test_revoked_credential_stays_revoked(self):
        issued=self.cloud.run("credential.issue",principal="agent:claude:mac").result
    ...
    def test_events_never_include_a_fingerprint_derived_from_a_leaked_secret_marker(self):
        ...
        self.cloud.run("credential.issue",principal="agent:claude:mac",fingerprint=fingerprint("do-not-leak-this-value"))
