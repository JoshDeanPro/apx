...
class ActorCredentialStoreTests(unittest.TestCase):
    ...
    def test_issue_defaults_to_active_with_no_plaintext(self):
        record=self.store.issue("agent:worker:node-1",fingerprint=fingerprint("super-secret-value"))
        self.assertEqual(record["state"],"active")
        self.assertNotIn("super-secret-value",json.dumps(record))

    def test_invalid_type_rejected(self):
        with self.assertRaises(ActorCredentialError): self.store.issue("agent:worker:node-1",type="not-a-real-type")

    def test_rotate_keeps_old_active_material_marked_rotating_not_revoked(self):
        original=self.store.issue("agent:worker:node-1",fingerprint=fingerprint("v1"))
    ...
    def test_cannot_rotate_a_non_active_credential(self):
        original=self.store.issue("agent:worker:node-1")
    ...
    def test_revoke_is_immediate_and_independent_of_rotation(self):
        record=self.store.issue("agent:worker:node-1")
        revoked=self.store.revoke(record["id"])
        self.assertEqual(revoked["state"],"revoked")

    def test_list_for_filters_by_principal(self):
        self.store.issue("agent:worker:node-1")
        self.store.issue("human:operator")
        self.assertEqual(len(self.store.list_for("agent:worker:node-1")),1)

    def test_expires_lazily_computed_on_read(self):
        from datetime import datetime, timedelta, timezone
        past=(datetime.now(timezone.utc)-timedelta(days=1)).isoformat()
        record=self.store.issue("agent:worker:node-1",expires=past)
        self.assertEqual(self.store.inspect(record["id"])["state"],"expired")

    def test_persists_across_instances(self):
        record=self.store.issue("agent:worker:node-1",fingerprint="fp1")
...
class ActorCredentialActionsTests(unittest.TestCase):
    ...
    def test_issue_rotate_confirm_revoke_via_actions(self):
        issued=self.cloud.run("credential.issue",principal="agent:worker:node-1",fingerprint="fp1").result
    ...
    def test_revoked_credential_stays_revoked(self):
        issued=self.cloud.run("credential.issue",principal="agent:worker:node-1").result
    ...
    def test_events_never_include_a_fingerprint_derived_from_a_leaked_secret_marker(self):
        ...
        self.cloud.run("credential.issue",principal="agent:worker:node-1",fingerprint=fingerprint("do-not-leak-this-value"))
