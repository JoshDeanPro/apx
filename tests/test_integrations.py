...
from localcloud import LocalCloud, Resource, VersionInfo
from localcloud.cli import main as cli_main
from localcloud.integrations.cloudflare import Plugin as CloudflarePlugin
from localcloud.integrations.godaddy import Plugin as GoDaddyPlugin
from localcloud.integrations.databases.aws import from_db_instance
from localcloud.integrations.databases.digitalocean import from_cluster
from localcloud.integrations.databases.models import parse_database_url, redact_database_url
from localcloud.integrations.databases.mysql import discover as discover_mysql
from localcloud.integrations.databases.postgres import discover as discover_postgres
from localcloud.integrations.databases.supabase import from_project
from localcloud.protocol import MCPServer
from localcloud.service_managers import manager_for
from localcloud.system import connection_status, scheduler_list, tailscale_status
from localcloud.transports import CommandResult, FallbackTransport
...
class IntegrationTests(unittest.TestCase):
    ...
    def test_groups_tags_and_multiple_membership(self):
        with tempfile.TemporaryDirectory() as directory:
            cloud=LocalCloud(write_config(Path(directory)),plugins=False)
    ...
    def test_credential_groups_health(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.dict(os.environ,{},clear=False):
                os.environ.pop("LC_TEST_CF",None); health=LocalCloud(path,plugins=False).credentials.health()[0]
    ...
    def test_database_action_discovers_native_services_without_clients(self):
        with tempfile.TemporaryDirectory() as directory:
            cloud=LocalCloud(write_config(Path(directory)))
    ...
    def test_connection_fallback(self):
        ...
        result=FallbackTransport(__import__("localcloud").Host("host","ssh","one"),[first,second]).run(["true"])
    ...
    def test_tailscale_discovery(self):
        ...
        with patch("localcloud.system.inspect_host",return_value=info),patch("localcloud.system.transport_for",return_value=transport):
            result=tailscale_status(__import__("localcloud").Host("mac","local"))
    ...
    def test_cron_systemd_timer_and_launchd_discovery(self):
        host=__import__("localcloud").Host("server","ssh","server")
        ...
        with patch("localcloud.system.inspect_host",return_value={"capabilities":caps}),patch("localcloud.system.transport_for",return_value=transport): result=scheduler_list(host)
