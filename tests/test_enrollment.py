...
class EnrollmentStoreTests(unittest.TestCase):
    ...
    def test_manual_mode_leaves_pending(self):
        record=self.store.request(machine_id="machine:mac",runtime="worker",mode="manual")
        self.assertEqual(record["status"],"pending")

    def test_disabled_mode_refuses_request(self):
        with self.assertRaises(EnrollmentError): self.store.request(machine_id="machine:mac",runtime="worker",mode="disabled")
    ...
    def test_automatic_mode_only_applies_when_explicitly_requested(self):
        ...
        manual=self.store.request(machine_id="machine:mac",runtime="worker",mode="manual")
        self.assertEqual(manual["status"],"pending")
        automatic=self.store.request(machine_id="machine:mac",runtime="worker",mode="automatic")
    ...
    def test_approve_deny_cancel(self):
        pending=self.store.request(machine_id="machine:mac",runtime="worker")
        approved=self.store.approve(pending["id"],resolved_by="human:operator")
        ...
        other=self.store.request(machine_id="machine:mac",runtime="worker")
        denied=self.store.deny(other["id"],resolved_by="human:operator")
        self.assertEqual(denied["status"],"denied")

        third=self.store.request(machine_id="machine:mac",runtime="worker")
    ...
    def test_list_filters_by_status(self):
        a=self.store.request(machine_id="m",runtime="worker")
        b=self.store.request(machine_id="m",runtime="reviewer")
        self.store.approve(a["id"])
        self.assertEqual([r["id"] for r in self.store.list(status="pending")],[b["id"]])

    def test_persists_across_instances(self):
        record=self.store.request(machine_id="machine:mac",runtime="worker")
...
class EnrollmentActionsTests(unittest.TestCase):
    ...
    def test_default_mode_is_manual_and_requires_approval(self):
        ...
        result=cloud.run("identity.enrollment.request",machine_id="machine:mac",runtime="worker")
    ...
    def test_enrollment_events_emitted(self):
        ...
        request=cloud.run("identity.enrollment.request",machine_id="machine:mac",runtime="worker").result
        cloud.run("identity.enrollment.approve",request_id=request["id"],approved_by="human:operator")
