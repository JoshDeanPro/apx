...
from apx import ActionRequest, ActionResult, Context, Event, APX, Resource, StructuredError, ActionReceipt, PreparedAction
...
class AXPTests(unittest.TestCase):
    def test_request_result_and_error_round_trip(self):
        request=ActionRequest("service.status",{"host":"server"},{"host":"server","service":"example"})
    ...
    def test_structured_execution_error(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertFalse(result.ok); self.assertEqual(result.error.code,"unavailable")
    ...
    def test_from_dict_filters_unknown_keys(self):
        # 1. StructuredError
        err_dict = {"code": "test_err", "message": "msg", "extra_field": "val"}
        err = StructuredError.from_dict(err_dict)
        self.assertEqual(err.code, "test_err")
        self.assertEqual(err.message, "msg")

        # 2. ActionReceipt
        receipt_dict = {"action": "action", "provider": "prov", "extra_field": "val", "side_effects": ["se1"]}
        receipt = ActionReceipt.from_dict(receipt_dict)
        self.assertEqual(receipt.action, "action")
        self.assertEqual(receipt.provider, "prov")
        self.assertEqual(receipt.side_effects, ("se1",))

        # 3. PreparedAction
        prepared_dict = {"action": "action", "extra_field": "val", "side_effects": ["se1"]}
        prepared = PreparedAction.from_dict(prepared_dict)
        self.assertEqual(prepared.action, "action")
        self.assertEqual(prepared.side_effects, ("se1",))
