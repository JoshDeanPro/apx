...
class OptionalExtensionTests(unittest.TestCase):
    ...
    def test_reward_requires_acceptance_and_bound_transaction_confirmation(self):
        ...
        self.assertEqual(rejected.status,"awaiting-approval")
    ...
    def test_daily_life_resources_validation(self):
        with self.assertRaises(ValueError):
            SubscriptionObservation("obs", "Provider", {"amount": "5"}, confidence=1.5)
        with self.assertRaises(ValueError):
            SubscriptionObservation("obs", "Provider", {"amount": "5"}, confidence=-0.1)
        with self.assertRaises(ValueError):
            FinancialResource("bank:test", "invalid_kind", "test", "Test").to_resource()
