...
from apx import APX
from apx.environment_ingest import ClaudeCodeSource, CodexSource, PLANNED_SOURCES, SOURCES
...
class SourceParsingTests(unittest.TestCase):
    def test_claude_code_source_missing_file(self):
        source = ClaudeCodeSource(Path("/nonexistent/.claude.json"))
        self.assertEqual(source.probe(), {"id": "claude_code", "found": False})
        self.assertEqual(source.mcp_servers(), {})

    def test_claude_code_source_parses_real_shape(self):
    ...
    def test_claude_code_source_handles_malformed_json(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / ".claude.json"
...
class EnvironmentSourcesActionTests(unittest.TestCase):
    ...
    def test_ingest_requires_confirmation(self):
        result = self.cloud.run("environment.ingest", actor=self.actor, source="claude_code")
...
class EnvironmentIngestActionTests(unittest.TestCase):
    ...
    real subprocess), not the real ~/.codex or ~/.claude.json -- proves the
