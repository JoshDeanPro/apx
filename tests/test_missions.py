...
class TaskLifecycleTests(unittest.TestCase):
    ...
    def test_claim_and_release(self):
        task=self.create_task()
        self.cloud.run("task.claim",task=task["id"],claimant="agent:claude:mac")
        conflict=self.cloud.run("task.claim",task=task["id"],claimant="agent:claude:vps")
        self.assertFalse(conflict.ok)
        self.cloud.run("task.release",task=task["id"],claimant="agent:claude:mac")
        self.assertTrue(self.cloud.run("task.claim",task=task["id"],claimant="agent:claude:vps").ok)
    ...
    def test_claim_with_no_actor_fails_instead_of_silently_no_op_claiming(self):
        ...
        self.assertTrue(self.cloud.run("task.claim",task=task["id"],claimant="agent:claude:mac").ok)
...
class ScopeChangeTests(unittest.TestCase):
    ...
    def test_request_and_approve(self):
        request=self.cloud.run("mission.scope_change.request",mission=self.mission["id"],reason="middleware can't support this",impact="auth architecture changes",requested_by="agent:claude:mac").result
...
class MissionScopedPermissionTests(unittest.TestCase):
    ...
    def test_denied_without_grant(self):
        result=self.cloud.run("service.restart",actor="agent:claude:mac",host="test",service="x")
    ...
    def test_mission_grant_allows_while_mission_active(self):
        self.cloud.run("mission.grant",mission=self.mission["id"],grantee="agent:claude:mac",action="service.restart")
        # proposed mission (not yet active) does not grant
        denied=self.cloud.run("policy.explain",subject="agent:claude:mac",requested_action="service.restart")
        self.assertFalse(denied.result["allowed"])
        self.cloud.run("mission.start",mission=self.mission["id"])
        allowed=self.cloud.run("policy.explain",subject="agent:claude:mac",requested_action="service.restart")
    ...
    def test_grant_expires_when_mission_ends(self):
        self.cloud.run("mission.grant",mission=self.mission["id"],grantee="agent:claude:mac",action="service.restart")
        self.cloud.run("mission.start",mission=self.mission["id"])
        self.assertTrue(self.cloud.run("policy.explain",subject="agent:claude:mac",requested_action="service.restart").result["allowed"])
        self.cloud.run("mission.cancel",mission=self.mission["id"],reason="no longer needed")
        self.assertFalse(self.cloud.run("policy.explain",subject="agent:claude:mac",requested_action="service.restart").result["allowed"])

    def test_grant_never_overrides_explicit_static_deny(self):
        self.cloud.run("mission.grant",mission=self.mission["id"],grantee="agent:claude:mac",action="host.shutdown")
        self.cloud.run("mission.start",mission=self.mission["id"])
        result=self.cloud.run("policy.explain",subject="agent:claude:mac",requested_action="host.shutdown")
    ...
    def test_actor_cannot_grant_a_permission_it_does_not_itself_hold(self):
        # agent:claude:mac only has "developer" (project.inspect); it must not be able to
        # self-escalate to secret.reveal just because it can call mission.grant at all.
        self.cloud.run("mission.start",mission=self.mission["id"])
        escalation=self.cloud.run("mission.grant",actor="agent:claude:mac",mission=self.mission["id"],grantee="agent:claude:mac",action="secret.reveal")
        self.assertFalse(escalation.ok)
        self.assertEqual(escalation.error.code,"permission_denied")
        self.assertFalse(self.cloud.run("policy.explain",subject="agent:claude:mac",requested_action="secret.reveal").result["allowed"])
    ...
    def test_actor_can_grant_a_permission_it_does_hold(self):
        ...
        result=self.cloud.run("mission.grant",mission=self.mission["id"],grantee="agent:claude:mac",action="project.inspect")
        self.assertTrue(result.ok)
...
class WorkContextAndResumeTests(unittest.TestCase):
    ...
    def test_work_current_is_scoped_not_the_whole_environment(self):
        self.cloud.run("task.claim",task=self.task["id"],claimant="agent:claude:mac")
        self.cloud.run("task.start",task=self.task["id"])
        context=self.cloud.run("work.current",subject="agent:claude:mac").result
