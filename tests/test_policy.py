...
class PolicyTests(unittest.TestCase):
    def test_disabled_engine_allows_everything(self):
        ...
        decision=eng.evaluate("agent::mac","project.deploy")
    ...
    def test_no_applicable_allow_denies_with_explainable_reason(self):
        eng=engine([{"id":"agent::mac","roles":["developer"]}],[{"name":"developer","allow":[{"action":"project.inspect"}]}])
        decision=eng.evaluate("agent::mac","project.deploy")
    ...
    def test_glob_action_prefix_matches(self):
        ...
        self.assertTrue(eng.evaluate("agent::mac","project.deploy").allowed)
        self.assertFalse(eng.evaluate("agent::mac","service.restart").allowed)

    def test_scoped_allow_respects_project_dimension(self):
        eng=engine([{"id":"agent::vps","roles":["deployer"]}],[{"name":"deployer","allow":[{"action":"service.restart","scope":{"project":["palisbot"]}}]}])
        self.assertTrue(eng.evaluate("agent::vps","service.restart",{"project":"palisbot"}).allowed)
        self.assertFalse(eng.evaluate("agent::vps","service.restart",{"project":"other"}).allowed)
        self.assertFalse(eng.evaluate("agent::vps","service.restart",{}).allowed)
    ...
    def test_explicit_deny_wins_over_allow(self):
        eng=engine(
            [{"id":"agent::home","roles":["developer"]}],
            [{"name":"developer","allow":[{"action":"file.write"},{"action":"project.deploy"}],"deny":[{"action":"project.deploy"}]}],
        )
        decision=eng.evaluate("agent::home","project.deploy")
        self.assertFalse(decision.allowed)
        self.assertIn("explicit deny",decision.reason)
        self.assertTrue(eng.evaluate("agent::home","file.write").allowed)

    def test_state_scoped_rule(self):
        eng=engine([{"id":"agent::vps","roles":["responder"]}],[{"name":"responder","allow":[{"action":"logs.read","scope":{"state":["incident","lockdown"]}}]}])
        self.assertFalse(eng.evaluate("agent::vps","logs.read",{},"normal").allowed)
        self.assertTrue(eng.evaluate("agent::vps","logs.read",{},"incident").allowed)
    ...
    def test_host_scope_matches_two_host_actions_by_either_endpoint(self):
        ...
        eng=engine(
            [{"id":"agent::mac","roles":["copier"]}],
            [{"name":"copier","allow":[{"action":"file.*"}],"deny":[{"action":"file.*","scope":{"host":["prod"]}}]}],
        )
        self.assertFalse(eng.evaluate("agent::mac","file.copy",{"source_host":"prod","destination_host":"dev"}).allowed)
        self.assertFalse(eng.evaluate("agent::mac","file.copy",{"source_host":"dev","destination_host":"prod"}).allowed)
        self.assertTrue(eng.evaluate("agent::mac","file.copy",{"source_host":"dev","destination_host":"staging"}).allowed)
        # single-host actions still work via the plain "host" target key
        self.assertFalse(eng.evaluate("agent::mac","file.sync",{"host":"prod"}).allowed)
    ...
    def test_might_allow_is_scope_agnostic_convenience_only(self):
        ...
        self.assertTrue(eng.might_allow("agent::vps","service.restart"))
        self.assertFalse(eng.evaluate("agent::vps","service.restart",{"project":"other"}).allowed)
