...
class ProtocolTests(unittest.TestCase):
    def setUp(self):
        ...
        self.client=APXClient(LocalClientTransport(self.session), client_context={"permissions": ["subscription.start", "subscription.inspect", "subscription.cancel", "subscription.resume", "account.sessions.revoke", "order.refund.request"], "authentication": ["session"], "actor_type": "human"})
    ...
    def test_confirmation_replay_and_intent_change_fail_closed(self):
        ...
        self.assertEqual(self.client.authorize(another.prepared_action_id,proof).status,"awaiting-approval")
    ...
    def test_durable_idempotency_receipt_and_status_survive_restart(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            first=ProviderSession(self.provider,state_path=database); durable=APXClient(LocalClientTransport(first), client_context={"permissions": ["subscription.start", "subscription.inspect", "subscription.cancel", "subscription.resume", "account.sessions.revoke", "order.refund.request"], "authentication": ["session"], "actor_type": "human"})
            ...
            restored=ProviderSession(self.provider,state_path=database); recovered=APXClient(LocalClientTransport(restored), client_context={"permissions": ["subscription.start", "subscription.inspect", "subscription.cancel", "subscription.resume", "account.sessions.revoke", "order.refund.request"], "authentication": ["session"], "actor_type": "human"})
    ...
    def test_long_running_operation_and_budget(self):
        ...
        session=ProviderSession(provider); client=APXClient(LocalClientTransport(session), client_context={"permissions": ["subscription.start", "subscription.inspect", "subscription.cancel", "subscription.resume", "account.sessions.revoke", "order.refund.request"], "authentication": ["session"], "actor_type": "human"})
...
class InteropTests(unittest.TestCase):
    def test_independent_typescript_client_over_http(self):
        ...
        starter=APXClient(LocalClientTransport(session), client_context={"permissions": ["subscription.start", "subscription.inspect", "subscription.cancel", "subscription.resume", "account.sessions.revoke", "order.refund.request"], "authentication": ["session"], "actor_type": "human"}); prepared=starter.prepare("subscription.start",input={"plan":"Plus"},actor="human")
    ...
    def test_python_client_against_independent_typescript_provider(self):
        ...
        try:
            port=json.loads(process.stdout.readline())["port"]; client=APXClient(HTTPClientTransport(f"http://127.0.0.1:{port}"), client_context={"permissions": ["subscription.start", "subscription.inspect", "subscription.cancel", "subscription.resume", "account.sessions.revoke", "order.refund.request"], "authentication": ["session"], "actor_type": "human"})
