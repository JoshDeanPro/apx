...
class AgentInstallTests(unittest.TestCase):
    ...
    def test_claude_code_project_scope_writes_slash_command_and_skill_no_mcp(self):
        result = install("claude-code", root=self.root)
        self.assertEqual(result["scope"], "project")
        command_path = self.root/".claude/commands/apx.md"
        skill_path = self.root/".claude/skills/apx/SKILL.md"
    ...
    def test_global_scope_writes_under_home_not_project(self):
        with tempfile.TemporaryDirectory() as fake_home:
            with mock.patch("pathlib.Path.home", return_value=Path(fake_home)):
                result = install("claude-code", root=self.root, global_scope=True)
            self.assertEqual(result["scope"], "user")
            self.assertFalse((self.root/".claude").exists())
            self.assertTrue((Path(fake_home)/".claude/commands/apx.md").exists())
            self.assertTrue((Path(fake_home)/".claude/skills/apx/SKILL.md").exists())
