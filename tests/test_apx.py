...
class APXTests(unittest.TestCase):
    ...
    def test_actor_threads_identically_through_python_and_mcp(self):
        python_result=self.cloud.run("actor.whoami",subject="agent:claude:mac")
        server=MCPServer(self.cloud,actor="agent:claude:mac")
        request={"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"actor_whoami","arguments":{"subject":"agent:claude:mac"}}}
        ...
        self.assertEqual(python_result.result["actor"],"agent:claude:mac")
    ...
    def test_unconfigured_default_actor_is_open_for_every_caller(self):
        ...
        self.assertTrue(self.cloud.run("host.info",host="test",actor="agent:claude:anything").ok)
